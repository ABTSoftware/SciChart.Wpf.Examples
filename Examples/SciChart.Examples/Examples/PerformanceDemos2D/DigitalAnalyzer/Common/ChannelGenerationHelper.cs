...
namespace SciChart.Examples.Examples.PerformanceDemos2D.DigitalAnalyzer.Common
{
    public class ChannelGenerationHelper
    {
        ...
        public ChannelViewModel GenerateAnalogChannel(double xStart, double xStep, double[] analogData, int index = 0)
        {
            UniformDataManager.GenerateAnalogData(analogData);
            var dataSeries = new UniformXyDataSeries<double>(xStart, xStep, analogData);
