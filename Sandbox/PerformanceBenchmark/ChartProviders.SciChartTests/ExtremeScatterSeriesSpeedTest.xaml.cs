...
{
    public partial class ExtremeScatterSeriesSpeedTest : UserControl, ISpeedTest
    {
        ...
        public ExtremeScatterSeriesSpeedTest()
        {
            InitializeComponent();
            sciChart.Loaded += (s, e) =>
            {
                infoTextAnnotation.Text = string.Format("Renderer: {0}, C++ Resampler: {1}, Extreme Drawing: {2}, Extreme Scatter: {3}", sciChart.RenderSurface.GetType().Name, PerformanceHelper.GetEnableExtremeResamplers(sciChart), PerformanceHelper.GetEnableExtremeDrawingManager(sciChart), true);
            };
        }
        ...
        public void Execute(TestParameters testParameters, TimeSpan duration, Action<double> fpsResult)
        {
            ...
            scatterSeries.PaletteProvider = new TestPaletteProvider();
        ...
        private void OnAppendData(int pointCount)
        {
            ...
            _xyDataSeries = new XyDataSeries<double, double>(pointCount)
            {
